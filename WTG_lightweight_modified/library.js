...
// Sleep duration ranges keyed by semantic time-of-day bucket.
// These are only used when state.currentTime is already a precise numeric clock.
const sleepDurationRanges = {
  dawn: {minHours: 1, maxHours: 3},
  morning: {minHours: 1, maxHours: 4},
  afternoon: {minHours: 1, maxHours: 3},
  evening: {minHours: 3, maxHours: 6},
  night: {minHours: 6, maxHours: 9},
  unknown: {minHours: 6, maxHours: 8}
};
...
/**
 * Classifies a precise clock time into a broad time-of-day bucket.
 * This is used for sleep-duration selection only; it does not replace
 * text parsing or descriptive time normalization elsewhere in the system.
 * @param {string} timeStr - Precise time string in hh:mm AM/PM format
 * @returns {string} One of dawn, morning, afternoon, evening, night, unknown
 */
function getTimeSemanticBucket(timeStr) {
  if (!timeStr || timeStr === 'Unknown' || !/\d/.test(timeStr)) return 'unknown';
  const {hour, min} = parseTime(timeStr);
  if (Number.isNaN(hour) || Number.isNaN(min)) return 'unknown';
  const totalMinutes = hour * 60 + min;
  if (totalMinutes >= 300 && totalMinutes < 420) return 'dawn';
  if (totalMinutes >= 420 && totalMinutes < 720) return 'morning';
  if (totalMinutes >= 720 && totalMinutes < 1020) return 'afternoon';
  if (totalMinutes >= 1020 && totalMinutes < 1260) return 'evening';
  return 'night';
}

/**
 * Generates a randomized sleep duration for a given time-of-day bucket.
 * Minutes remain random so wake-up times are not locked to exact hours.
 * @param {string} bucket - Semantic bucket returned by getTimeSemanticBucket
 * @returns {Object} Duration object compatible with addToTurnTime
 */
function getSleepDurationFromBucket(bucket) {
  const range = sleepDurationRanges[bucket] || sleepDurationRanges.unknown;
  return {
    hours: Math.floor(Math.random() * (range.maxHours - range.minHours + 1)) + range.minHours,
    minutes: Math.floor(Math.random() * 60)
  };
}

/**
 * Generates a conservative fallback wake time when no reliable current clock exists.
 * We keep the old "wake next day" compatibility, but avoid hardcoding a single 8:00 AM result.
 * @returns {string} Wake time string in hh:mm AM/PM format
 */
function getUnknownSleepFallbackWakeTime() {
  const fallbackHour = Math.floor(Math.random() * 3) + 7; // 7-9 AM keeps legacy behavior stable
  const fallbackMinute = Math.floor(Math.random() * 60);
  return `${fallbackHour}:${String(fallbackMinute).padStart(2, '0')} AM`;
}
 ...
 * @param {string[]} previousDefaultDates - Older default dates that should also be treated as placeholders
 */
function updateAllStoryCardTimestamps(newDate, newTime, previousDefaultDates = []) {
  const timestamp = `${newDate} ${newTime}`;
  const placeholderDates = Array.from(new Set(
    [WTG_SCENE_START_DATE, ...(previousDefaultDates || [])].filter(Boolean)
  ));
  const defaultTimestampRegex = placeholderDates.length > 0
    ? new RegExp(
        `(Discovered on|Met on|Visited) (?:${placeholderDates.map(escapeRegExp).join('|')})\\s+[\\d:]+ [AP]M`
      )
    : null;
  ...
  for (let i = 0; i < storyCards.length; i++) {
    ...
    if (card.entry && (card.entry.includes("Discovered on") || card.entry.includes("Met on") || card.entry.includes("Visited"))) {
      ...
      } else if (defaultTimestampRegex && placeholderDates.some(date => card.entry.includes(date))) {
        // Replace default date timestamp with new timestamp
        card.entry = card.entry.replace(defaultTimestampRegex, `$1 ${timestamp}`);
      }
    }
  }
}
...
function estimateDynamicMinutes(turnText, charCount = 0, similarity = 0.5) {
  ...
  // Use single-word passage cues here instead of longer phrases so the heuristic stays
  // easy to maintain and does not depend on exact sentence templates.
  const longPassagePattern =
    /\b(later|eventually|afterward|afterwards|meanwhile|dawn|noon|dusk|midnight|tomorrow|hours?|minutes?)\b/;
...
function onInput_WTG(text) {
  ...
  if (text.trim().toLowerCase() === '[sleep]') {
    if (state.currentTime !== 'Unknown' && /\d/.test(state.currentTime)) {
      // When a precise clock exists, choose sleep length from the current time-of-day bucket
      // instead of forcing every sleep to behave like a full night's rest.
      const bucket = getTimeSemanticBucket(state.currentTime);
      let add = getSleepDurationFromBucket(bucket);
      ...
      // If no reliable current clock exists, keep the old compatibility behavior of waking on
      // the next day, but randomize the wake time slightly so it is not locked to one exact minute.
      state.turnTime = {years:0, months:0, days:0, hours:0, minutes:0, seconds:0};
      state.turnTime = addToTurnTime(state.turnTime, {days: 1});
      state.startingTime = getUnknownSleepFallbackWakeTime();
      const {currentDate, currentTime} = computeCurrent(state.startingDate, state.startingTime, state.turnTime);
      state.currentDate = currentDate;
      state.currentTime = currentTime;
      const ttMarker = formatTurnTime(state.turnTime);
  ...
  // Handle [settime], [advance], [reset] and other bracket commands.
    ...
    if (trimmedText.match(/^\[(.+?)\]$/)) {
      ...
      if (command === 'settime') {
        ...
        const previousStartingDate = state.startingDate;
        if (dateStr) {
          ...
          if (isValidDate(month, day, year)) {
            ...
            updateAllStoryCardTimestamps(state.currentDate, state.currentTime, [previousStartingDate]);
